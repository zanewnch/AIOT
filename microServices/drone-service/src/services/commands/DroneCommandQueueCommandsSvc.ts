 ...
 *
 * @module DroneCommandQueueCommandsService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
...
} from '../../types/services/IDroneCommandQueueSvc.js';
...
const logger = createLogger('DroneCommandQueueCommandsService');
 ...
 *
 * @class DroneCommandQueueCommandsService
 * @since 1.0.0
 */
@injectable()
export class DroneCommandQueueCommandsSvc {
    ...
    constructor(
        ...
        @inject(TYPES.DroneCommandQueueCommandsRepo) private readonly droneCommandQueueCommandsRepo: DroneCommandQueueCommandsRepo
    ) {}
    ...
    updateDroneCommandQueue = async (id: number, data: Partial<DroneCommandQueueCreationAttributes>): Promise<DroneCommandQueueAttributes | null> => {
        try {
            ...
            // 檢查佇列是否存在
            const existingQueue = await this.droneCommandQueueCommandsRepo.findById(id);
            if (!existingQueue) {
    ...
    executeQueue = async (id: number): Promise<QueueExecutionResult> => {
        try {
            ...
            const queue = await this.droneCommandQueueCommandsRepo.findById(id);
            if (!queue) {
                return {
                    success: false,
