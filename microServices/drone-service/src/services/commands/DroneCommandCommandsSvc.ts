 ...
 *
 * @module DroneCommandCommandsService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
...
import type {
    ...
    BatchCommandResult
} from '../../types/services/IDroneCommandSvc.js';
...
import { DroneCommandCommandsRepo } from '../../repo/commands/DroneCommandCommandsRepo.js';

const logger = createLogger('DroneCommandCommandsService');
 ...
 *
 * @class DroneCommandCommandsService
 * @since 1.0.0
 */
@injectable()
export class DroneCommandCommandsSvc {
    constructor(
        ...
        private readonly droneCommandQueriesSvc: DroneCommandQueriesSvc,
        @inject(TYPES.DroneCommandCommandsRepo)
        private readonly droneCommandCommandsRepo: DroneCommandCommandsRepo
    ) {}
    ...
    createCommand = async (data: DroneCommandCreationAttributes): Promise<CommandExecutionResult> => {
        try {
            ...
            const createdCommand = await this.droneCommandCommandsRepo.create(commandData);
    ...
    updateCommand = async (id: number, data: Partial<DroneCommandCreationAttributes>): Promise<DroneCommandAttributes | null> => {
        try {
            ...
            const existingCommand = await this.droneCommandCommandsRepo.findById(id);
            if (!existingCommand) {
            ...
            const updatedCommand = await this.droneCommandCommandsRepo.update(id, data);
    ...
    deleteCommand = async (id: number): Promise<boolean> => {
        try {
            ...
            await this.droneCommandCommandsRepo.delete(id);
    ...
    executeCommand = async (commandId: number): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.droneCommandCommandsRepo.markAsExecuting(commandId);
    ...
    completeCommand = async (commandId: number): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.droneCommandCommandsRepo.markAsCompleted(commandId);
    ...
    failCommand = async (commandId: number, errorMessage: string): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.droneCommandCommandsRepo.markAsFailed(commandId, errorMessage);
    ...
    cancelCommand = async (commandId: number, reason: string): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.droneCommandCommandsRepo.markAsFailed(commandId, `已取消: ${reason}`);
    ...
    sendTakeoffCommand = async (droneId: number, issuedBy: number, commandData: { altitude: number; speed?: number }): Promise<CommandExecutionResult> => {
        try {
            ...
            // 驗證起飛指令參數
            if (!commandData.altitude || commandData.altitude < 1 || commandData.altitude > 500) {
                return {
                    success: false,
                    command: null,
                    message: '起飛指令參數無效',
    ...
    handleTimeoutCommands = async (timeoutMinutes: number): Promise<number> => {
        try {
            ...
            // TODO: 實現超時指令查詢邏輯
            const timeoutCommands: any[] = [];
            let handledCount = 0;

            for (const command of timeoutCommands) {
                try {
                        await this.droneCommandCommandsRepo.markAsFailed(command.id, '指令執行超時');
                    handledCount++;
                } catch (error) {
                    }
            }
    ...
    cleanupOldCommands = async (beforeDate: Date, onlyCompleted: boolean = true): Promise<number> => {
        try {
            ...
            let deletedCount = 0;
            if (onlyCompleted) {
                deletedCount = await this.droneCommandCommandsRepo.deleteCompletedBefore(beforeDate);
            } else {
                deletedCount = await this.droneCommandCommandsRepo.deleteBeforeDate(beforeDate);
            }
    ...
    batchUpdateCommandStatus = async (commandIds: number[], status: DroneCommandStatus, errorMessage?: string): Promise<number> => {
        try {
            ...
            for (const commandId of commandIds) {
                try {
                    const result = await this.droneCommandCommandsRepo.updateStatus(commandId, status, errorMessage);
                    if (result) {
                        updatedCount++;
                    }
                } catch (error) {
    ...
    retryFailedCommand = async (commandId: number, issuedBy: number): Promise<CommandExecutionResult> => {
        try {
            ...
            const originalCommand = await this.droneCommandCommandsRepo.findById(commandId);
            if (!originalCommand) {
                return {
                    success: false,
                    command: null,
     ...
     * @private
     */
    private validateCommandData = (data: DroneCommandCreationAttributes): boolean => {
        ...
        if (!data.issued_by || data.issued_by <= 0) {
            return false;
        }
