 ...
 */
@injectable()
export class DroneCommandQueueCommandsService {
    private queryService: DroneCommandQueueQueriesService;
    private static idCounter = 3; // 模擬 ID 計數器，從 3 開始因為查詢服務有 1,2

    constructor() {
        this.queryService = new DroneCommandQueueQueriesService();
        // TODO: 注入 Repositorysitorysitory 依賴當 Repositorysitorysitory 創建後
    }
    ...
    createDroneCommandQueue = async (data: DroneCommandQueueCreationAttributes): Promise<DroneCommandQueueAttributes> => {
        try {
            ...
            const newQueue: DroneCommandQueueAttributes = {
                id: DroneCommandQueueCommandsService.idCounter++,
                name: data.name,
