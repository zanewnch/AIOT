...
import { DroneCommandQueriesService } from '../queries/DroneCommandQueriesService.js';
 ...
 */
@injectable()
export class DroneCommandCommandsService {
    constructor(
        @inject(TYPES.DroneCommandQueriesService)
        private readonly queryService: DroneCommandQueriesService,
        @inject(TYPES.DroneCommandCommandsRepositorysitorysitory)
        private readonly commandRepositorysitorysitory: IDroneCommandRepositorysitorysitory
    ) {}
    ...
    createCommand = async (data: DroneCommandCreationAttributes): Promise<CommandExecutionResult> => {
        try {
            ...
            const createdCommand = await this.commandRepositorysitorysitory.create(commandData);
    ...
    updateCommand = async (id: number, data: Partial<DroneCommandCreationAttributes>): Promise<DroneCommandAttributes | null> => {
        try {
            ...
            const updatedCommand = await this.commandRepositorysitorysitory.update(id, data);
    ...
    deleteCommand = async (id: number): Promise<boolean> => {
        try {
            ...
            await this.commandRepositorysitorysitory.delete(id);
    ...
    executeCommand = async (commandId: number): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.commandRepositorysitorysitory.markAsExecuting(commandId);
    ...
    completeCommand = async (commandId: number): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.commandRepositorysitorysitory.markAsCompleted(commandId);
    ...
    failCommand = async (commandId: number, errorMessage: string): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.commandRepositorysitorysitory.markAsFailed(commandId, errorMessage);
    ...
    cancelCommand = async (commandId: number, reason: string): Promise<CommandExecutionResult> => {
        try {
            ...
            const command = await this.commandRepositorysitorysitory.markAsFailed(commandId, `已取消: ${reason}`);
    ...
    handleTimeoutCommands = async (timeoutMinutes: number): Promise<number> => {
        try {
            ...
            for (const command of timeoutCommands) {
                try {
                    await this.commandRepositorysitorysitory.markAsFailed(command.id, '指令執行超時');
                    handledCount++;
                } catch (error) {
                    }
            }
    ...
    cleanupOldCommands = async (beforeDate: Date, onlyCompleted: boolean = true): Promise<number> => {
        try {
            ...
            let deletedCount = 0;
            if (onlyCompleted) {
                deletedCount = await this.commandRepositorysitorysitory.deleteCompletedBefore(beforeDate);
            } else {
                deletedCount = await this.commandRepositorysitorysitory.deleteBeforeDate(beforeDate);
            }
    ...
    batchUpdateCommandStatus = async (commandIds: number[], status: DroneCommandStatus, errorMessage?: string): Promise<number> => {
        try {
            ...
            for (const commandId of commandIds) {
                try {
                    const result = await this.commandRepositorysitorysitory.updateStatus(commandId, status, errorMessage);
                    if (result) {
                        updatedCount++;
                    }
                } catch (error) {
