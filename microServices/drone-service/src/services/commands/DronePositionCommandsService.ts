 ...
 */
@injectable()
export class DronePositionCommandsService {
    private dronePositionRepositorysitory: IDronePositionRepositorysitory; // 組合介面

    constructor(
        @inject(TYPES.DronePositionCommandsRepositorysitorysitory) 
        private readonly commandsRepositorysitory: DronePositionCommandsRepositorysitorysitory,
        
        @inject(TYPES.DronePositionQueriesRepositorysitory) 
        private readonly queriesRepositorysitory: DronePositionQueriesRepositorysitory,
        
        @inject(TYPES.DronePositionQueriesService) 
        private readonly queryService: DronePositionQueriesService
    ) {
        // 創建組合repository
        this.dronePositionRepositorysitory = Object.assign(
        ...
        ) as IDronePositionRepositorysitory;
    }
    ...
    createDronePosition = async (data: DronePositionCreationAttributes): Promise<DronePositionAttributes> => {
        try {
            ...
            const dronePosition = await this.dronePositionRepositorysitorysitory.create(data);
    ...
    updateDronePosition = async (id: number, data: Partial<DronePositionCreationAttributes>): Promise<DronePositionAttributes> => {
        try {
            ...
            const updatedDronePosition = await this.dronePositionRepositorysitorysitory.update(id, data);
    ...
    deleteDronePosition = async (id: number): Promise<void> => {
        try {
            ...
            await this.dronePositionRepositorysitorysitory.delete(id);
    ...
    createDronePositionsBatch = async (positions: DronePositionCreationAttributes[]): Promise<DronePositionAttributes[]> => {
        try {
            ...
            for (const position of positions) {
                const result = await this.dronePositionRepositorysitorysitory.create(position);
                results.push(result);
            }
