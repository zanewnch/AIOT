 ...
 *
 * @module DroneStatusCommandsService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
...
const logger = createLogger('DroneStatusCommandsService');
 ...
 *
 * @class DroneStatusCommandsService
 * @since 1.0.0
 */
@injectable()
export class DroneStatusCommandsService {
    constructor(
        @inject(TYPES.DroneStatusQueriesService)
        private readonly queryService: DroneStatusQueriesService,
        @inject(TYPES.DroneStatusCommandsRepositorysitorysitory)
        private readonly droneStatusRepositorysitory: IDroneStatusRepositorysitorysitory
    ) {}
    ...
    createDroneStatus = async (data: DroneStatusCreationAttributes): Promise<DroneStatusAttributes> => {
        try {
            ...
            const droneStatus = await this.droneStatusRepositorysitory.create(data);
return droneStatus;
        } catch (error) {
            throw error;
        }
    ...
    updateDroneStatus = async (id: number, data: Partial<DroneStatusCreationAttributes>): Promise<DroneStatusAttributes> => {
        try {
            ...
            const updatedDroneStatus = await this.droneStatusRepositorysitory.update(id, data);
...
return updatedDroneStatus;
    ...
    deleteDroneStatus = async (id: number): Promise<void> => {
        try {
            ...
            await this.droneStatusRepositorysitory.delete(id);
} catch (error) {
            throw error;
        }
    }
    ...
    updateDroneStatusOnly = async (id: number, status: DroneStatus): Promise<DroneStatusAttributes> => {
        try {
            ...
            const updatedDroneStatus = await this.droneStatusRepositorysitory.updateStatus(id, status);
