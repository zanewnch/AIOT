 ...
 *
 * @module DroneCommandsArchiveCommandsService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
...
const logger = createLogger('DroneCommandsArchiveCommandsService');
 ...
 *
 * @class DroneCommandsArchiveCommandsService
 * @since 1.0.0
 */
@injectable()
export class DroneCommandsArchiveCommandsService {
    constructor(
        @inject(TYPES.DroneCommandsArchiveCommandsRepositorysitorysitory)
        private readonly archiveRepositorysitory: IDroneCommandsArchiveRepositorysitorysitory,
    ...
    createCommandArchive = async (data: DroneCommandsArchiveCreationAttributes): Promise<DroneCommandsArchiveAttributes> => {
        try {
            ...
            const archive = await this.archiveRepositorysitory.insert(data);
    ...
    archiveCommandsBefore = async (beforeDate: Date, batchSize: number = 50): Promise<ArchiveOperationResult> => {
        try {
            ...
            // 由於沒有活躍指令表的 Repositorysitorysitory，這裡提供架構示例
