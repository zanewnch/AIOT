 ...
 *
 * @module DroneCommandQueriesService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
 ...
 * 所有方法都是唯讀操作，不會修改系統狀態。
 *
 * @class DroneCommandQueriesService
 * @since 1.0.0
 */
@injectable()
export class DroneCommandQueriesSvc {
    constructor(
        ...
        private readonly droneCommandQueriesRepo: DroneCommandQueriesRepo
    ) {}
    ...
    getAllCommandsPaginated = async (
        ...
        try {
            ...
            const result = await this.droneCommandQueriesRepo.findPaginated(pagination);
    ...
    getCommandsByDroneIdPaginated = async (
        ...
        try {
            ...
            const result = await this.droneCommandQueriesRepo.findByDroneIdPaginated(droneId, pagination);
            const paginatedResponse = DtoMapper.toPaginatedDroneCommandResponse(result);
    ...
    getCommandsByStatusPaginated = async (
        ...
        try {
            ...
            const result = await this.droneCommandQueriesRepo.findByStatusPaginated(status, pagination);
            const paginatedResponse = DtoMapper.toPaginatedDroneCommandResponse(result);
