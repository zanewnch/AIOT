 ...
 */
@injectable()
export class DroneStatusQueriesService {
    private droneStatusRepositorysitory: DroneStatusQueriesRepositorysitory;

    constructor(
        @inject(TYPES.DroneStatusQueriesRepositorysitory) droneStatusRepositorysitory: DroneStatusQueriesRepositorysitory
    ) {
        this.droneStatusRepositorysitory = droneStatusRepositorysitory;
    }
    ...
    getAllStatusesPaginated = async (
        ...
        try {
            ...
            const result = await this.droneStatusRepositorysitory.findPaginated(pagination);
    ...
    getStatusesByStatusPaginated = async (
        ...
        try {
            ...
            const result = await this.droneStatusRepositorysitory.findByStatusPaginated(status, pagination);
            const paginatedResponse = DtoMapper.toPaginatedDroneStatusResponse(result);
    ...
    getStatusesByDroneIdPaginated = async (
        ...
        try {
            ...
            const result = await this.droneStatusRepositorysitory.findByDroneIdPaginated(droneId, pagination);
            const paginatedResponse = DtoMapper.toPaginatedDroneStatusResponse(result);
    ...
    isDroneSerialExists = async (serial: string): Promise<boolean> => {
        try {
            ...
            // 假設在無人機狀態表中檢查
            const result = await this.droneStatusRepositorysitory.findBySerial(serial);
            return result !== null;
        } catch (error) {
    ...
    getDroneStatusById = async (id: number): Promise<any> => {
        try {
            ...
            const result = await this.droneStatusRepositorysitory.findById(id);
            if (!result) {
                return null;
            }
