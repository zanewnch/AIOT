/**
 * @fileoverview Winston 日誌配置 - DRONE 服務
 * 
 * 提供 DRONE 服務的日誌記錄功能，支援多種輸出格式和日誌級別。
 * 包含檔案輪轉、彩色輸出和錯誤追蹤等功能。
 * 
 * 環境配置策略：
...
// 日誌輸出目錄
const logDir = path.join(__dirname, '../../../logs/drone');
...
const customFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, service, ...meta }: any) => {
    let logMessage = `${timestamp} [${service || 'DRONE'}] ${level.toUpperCase()}: ${message}`;
...
const consoleFormat = winston.format.combine(
  ...
  winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }),
  winston.format.printf(({ timestamp, level, message, service, ...meta }: any) => {
    let logMessage = `${timestamp} [${service || 'DRONE'}] ${level}: ${message}`;
...
const fileRotateTransport = new DailyRotateFile({
  filename: path.join(logDir, 'drone-app-%DATE%.log'),
...
const errorFileRotateTransport = new DailyRotateFile({
  filename: path.join(logDir, 'drone-error-%DATE%.log'),
  datePattern: 'YYYY-MM-DD',
  maxSize: '20m',
  maxFiles: '30d',
  format: customFormat,
...
const getExceptionHandlers = (): winston.transport[] => {
  const handlers: winston.transport[] = [
    new winston.transports.File({ 
      filename: path.join(logDir, 'drone-exceptions.log'),
      format: customFormat 
    })
  ];
...
const getRejectionHandlers = (): winston.transport[] => {
  const handlers: winston.transport[] = [
    new winston.transports.File({ 
      filename: path.join(logDir, 'drone-rejections.log'),
      format: customFormat 
    })
  ];
...
const logger = winston.createLogger({
  ...
  format: customFormat,
  defaultMeta: { service: 'AIOT-DRONE' },
...
logger.info('🚀 DRONE Winston Logger initialized', {
...
export function createLogger(service: string): winston.Logger {
  return logger.child({ service: `DRONE-${service}` });
}
