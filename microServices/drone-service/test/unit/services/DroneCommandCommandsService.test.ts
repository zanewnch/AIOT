 ...
 * 
 * 測試 DroneCommandCommandsService 類別的所有功能，包含：
 * - 指令創建和批量創建測試
 * - 指令更新和刪除測試
 * - 各種飛行指令發送測試
 * - 指令狀態管理測試
...
import { DroneCommandQueriesService } from '../../../src/services/queries/DroneCommandQueriesService.js';
import { DroneCommandCommandsRepositorysitory } from '../../../src/repo/commands/DroneCommandCommandsRepository.js';
import { DroneCommandQueriesRepository } from '../../../src/repo/queries/DroneCommandQueriesRepository.js';
...
// Mock repositories
jest.mock('../../../src/repo/commands/DroneCommandCommandsRepository.js');
jest.mock('../../../src/repo/queries/DroneCommandQueriesRepository.js');

describe('DroneCommandCommandsService', () => {
    let service: DroneCommandCommandsService;
    let mockQueryService: jest.Mocked<DroneCommandQueriesService>;
    let mockCommandsRepositorysitory: jest.Mocked<DroneCommandCommandsRepositorysitory>;
    ...
    beforeEach(() => {
        ...
        service = new DroneCommandCommandsService(mockQueryService);
    ...
    describe('createCommand', () => {
        it('應該成功創建單一指令', async () => {
            ...
            mockCommandsRepositorysitory.create.mockResolvedValue(expectedCommand);
    ...
    describe('updateCommand', () => {
        it('應該成功更新指令', async () => {
            ...
            mockCommandsRepositorysitory.update.mockResolvedValue(updatedCommand);

            const result = await service.updateCommand(commandId, updateData);

            expect(result).toEqual(updatedCommand);
            expect(mockCommandsRepositorysitory.update).toHaveBeenCalledWith(commandId, updateData);
        });
        ...
        it('應該在找不到指令時返回 null', async () => {
            ...
            mockCommandsRepositorysitory.update.mockResolvedValue(null);
    ...
    describe('deleteCommand', () => {
        it('應該成功刪除指令', async () => {
            ...
            expect(mockCommandsRepositorysitory.delete).toHaveBeenCalledWith(commandId);
        });

        it('應該在找不到指令時返回 false', async () => {
            const commandId = 999;
            mockCommandsRepositorysitory.delete.mockResolvedValue(false);
    ...
    describe('指令狀態管理測試', () => {
        describe('executeCommand', () => {
            it('應該成功執行指令', async () => {
                ...
                expect(mockCommandsRepositorysitory.update).toHaveBeenCalledWith(commandId, {
                    status: DroneCommandStatus.EXECUTING,
        ...
        describe('completeCommand', () => {
            it('應該成功完成指令', async () => {
                ...
                mockQueriesRepository.findById.mockResolvedValue(existingCommand);
                mockCommandsRepositorysitory.update.mockResolvedValue(completedCommand);
        ...
        describe('failCommand', () => {
            it('應該成功標記指令失敗', async () => {
                ...
                mockQueriesRepository.findById.mockResolvedValue(existingCommand);
                mockCommandsRepositorysitory.update.mockResolvedValue(failedCommand);
        ...
        describe('cancelCommand', () => {
            it('應該成功取消指令', async () => {
                ...
                mockQueriesRepository.findById.mockResolvedValue(existingCommand);
                mockCommandsRepositorysitory.update.mockResolvedValue(cancelledCommand);
    ...
    describe('retryFailedCommand', () => {
        it('應該成功重試失敗的指令', async () => {
            ...
            mockCommandsRepositorysitory.create.mockResolvedValue(newCommand);
