/**
 * @fileoverview MongoDB資料庫連接配置模組 - GENERAL 服務
 * 
 * 此模組提供了完整的MongoDB資料庫連接管理功能，專為 GENERAL 服務優化。
 * 包括連接建立、斷開、狀態監控和錯誤處理。
 * 
 * 主要特性：
 * - 自動重連機制
...
export const connectMongoDB = async (): Promise<typeof mongoose> => {
  try {
    if (mongoose.connection.readyState === 1) {
      console.log("📡 [GENERAL] 使用現有的 MongoDB 連接");
      return mongoose;
    }

    const mongoUrl = buildMongoUrl();
    console.log("[GENERAL] 正在連接 MongoDB...");
    console.log("[GENERAL] 連接字串:", mongoUrl.replace(/\/\/.*@/, "//***:***@"));

    await mongoose.connect(mongoUrl, mongoOptions);
    console.log("✅ [GENERAL] MongoDB 連接成功");

    mongoose.connection.on("error", (error: Error) => {
      console.error("❌ [GENERAL] MongoDB 連接錯誤:", error);
    });

    mongoose.connection.on("disconnected", () => {
      console.log("⚠️ [GENERAL] MongoDB 連接已斷開");
    });

    mongoose.connection.on("reconnected", () => {
      console.log("🔄 [GENERAL] MongoDB 重新連接成功");
    });

    return mongoose;
  } catch (error) {
    console.error("❌ [GENERAL] MongoDB 連接失敗:", error);
    process.exit(1);
  }
};
...
export const disconnectMongoDB = async (): Promise<void> => {
  try {
    if (mongoose.connection.readyState !== 0) {
      ...
      console.log("📴 [GENERAL] MongoDB 連接已關閉");
    }
  } catch (error) {
    console.error("❌ [GENERAL] MongoDB 斷開連接時發生錯誤:", error);
  }
};
