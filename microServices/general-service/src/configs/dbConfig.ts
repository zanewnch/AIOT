...
const getServiceType = (): ServiceType => {
  const serviceName = process.env.SERVICE_NAME || 'general-service';
...
export const createSequelizeInstance = (): Sequelize => {
  ...
  const sequelize = new Sequelize({
    // 展開資料庫配置物件的所有屬性
    ...config,
    // 註冊 General Service 相關模型到 Sequelize 實例中
