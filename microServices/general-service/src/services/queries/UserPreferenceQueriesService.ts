 ...
 *
 * @module UserPreferenceQueriesService
 * @author AIOT Team
 * @since 1.0.0
 * @version 1.0.0
 */
 ...
 *
 * @class UserPreferenceQueriesService
 * @since 1.0.0
 */
@injectable()
export class UserPreferenceQueriesService {
    constructor(
        @inject(TYPES.UserPreferenceQueriesRepository) 
     ...
     */
    getAllUserPreferences = async (limit: number = 100): Promise<UserPreferenceAttributes[]> => {
        try {
     ...
     */
    getUserPreferenceById = async (id: number): Promise<UserPreferenceAttributes | null> => {
        try {
     ...
     * @returns {Promise<UserPreferenceAttributes | null>} 用戶偏好設定或 null
     */
    getUserPreferenceByUserId = async (userId: number): Promise<UserPreferenceAttributes | null> => {
        try {
     ...
     */
    getUserPreferencesByTheme = async (
        theme: string, 
     ...
     * @returns {Promise<PaginatedData<UserPreferenceAttributes>>} 分頁的用戶偏好設定
     */
    getUserPreferencesWithPagination = async (
        pagination: PaginationParams = { page: 1, limit: 10 }
    ): Promise<PaginatedData<UserPreferenceAttributes>> => {
        try {
     ...
     */
    checkUserPreferenceExists = async (userId: number): Promise<boolean> => {
        try {
     ...
     */
    getUserPreferenceStatistics = async (): Promise<UserPreferenceStatistics> => {
        try {
     ...
     */
    getUserPreferenceCount = async (): Promise<number> => {
        try {
     ...
     */
    getUserPreferenceOrDefault = async (userId: number): Promise<UserPreferenceAttributes> => {
        try {
            const preference = await this.queriesRepository.selectByUserId(userId);
