...
export class JwtBlacklistService {
    ...
    private async setupRedisConnection(): Promise<void> {
        try {
            ...
            this.redisClient.on('connect', () => {
                logger.info('Redis Client Connected for JWT Blacklist');
            });

            await this.redisClient.connect();
        } catch (error) {
            logger.error('Failed to connect to Redis for JWT blacklist:', error);
            throw error;
        }
    }
    ...
    async addToBlacklist(token: string, expiresAt: number, reason: string = 'logout'): Promise<void> {
        try {
            ...
            logger.info(`JWT token added to blacklist: reason=${reason}, ttl=${ttl}s`);
        } catch (error) {
    ...
    async isBlacklisted(token: string): Promise<boolean> {
        try {
            ...
            if (result) {
                logger.debug('JWT token found in blacklist');
                return true;
            }
            
            return false;
        } catch (error) {
            logger.error('Failed to check JWT blacklist:', error);
    ...
    async removeFromBlacklist(token: string): Promise<boolean> {
        try {
            ...
            logger.info(`JWT token removed from blacklist: success=${result > 0}`);
            return result > 0;
        } catch (error) {
            logger.error('Failed to remove JWT token from blacklist:', error);
            throw error;
        }
    }
    ...
    async cleanupExpiredTokens(): Promise<number> {
        try {
            ...
            logger.info(`Cleaned up ${cleanedCount} expired JWT blacklist entries`);
            return cleanedCount;
        } catch (error) {
            logger.error('Failed to cleanup expired JWT blacklist entries:', error);
            throw error;
        }
    }
    ...
    async close(): Promise<void> {
        try {
            ...
            logger.info('JWT Blacklist Redis connection closed');
        } catch (error) {
            logger.error('Failed to close JWT Blacklist Redis connection:', error);
        }
    }
}