...
export class AuthQueriesService {
    private sessionQueriesService: SessionQueriesService; // 注入的 session 查詢服務

    constructor(
        @inject(TYPES.SessionQueriesService) sessionQueriesService: SessionQueriesService
    ) {
        this.sessionQueriesService = sessionQueriesService; // 建構子注入
    }
    ...
    public validateSession = async (token: string): Promise<SessionValidationResult> => {
        try {
            ...
            const sessionData = await this.sessionQueriesService.getUserSession(token); // 從 Redis 查詢會話
            if (!sessionData) {
    ...
    public sessionExists = async (token: string): Promise<boolean> => {
        try {
            ...
            const sessionData = await this.sessionQueriesService.getUserSession(token);
            return !!sessionData;
        } catch (error) {
