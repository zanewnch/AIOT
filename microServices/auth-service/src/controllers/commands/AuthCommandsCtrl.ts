/**
 * @fileoverview AuthCommandsController 控制器 - 檔案層級意圖說明
 *
 * 目的：此控制器負責處理所有寫入/命令型的認證 API 端點，遵循 CQRS 的命令端模式。
 * - 負責處理使用者登入、登出等會改變系統狀態的操作
 * - 從 HTTP 請求中解析和驗證輸入參數
 * - 委派具體的業務邏輯給 AuthCommandsService 服務層
...
const logger = createLogger('AuthCommandsController'); // 建立 module 專屬 logger
 ...
 *
 * @class AuthCommandsController
 * @example
 * ```typescript
 * // 透過 Inversify 容器注入使用
 * const authController = container.get<AuthCommandsController>(TYPES.AuthCommandsController);
 * ```
 *
 * @since 1.0.0
 * @public
...
export class class AuthCommandsCtrl {Ctrl { // 控制器類別
     ...
     *
     * @param authCommandsService - 認證命令服務實例，負責執行具體的認證業務邏輯
     */
    constructor(
        @inject(TYPES.AuthCommandsService) private readonly authCommandsService: AuthCommandsService // 注入命令服務實例，用於執行認證相關的業務邏輯
    ) {
    }
     ...
     * 3. 委派 AuthCommandsService 進行身份驗證
    ...
    public login = async (req: Request, res: Response, next: NextFunction): Promise<void> => { // 登入控制器方法
        try {
            ...
            const result = await this.authCommandsService.login({ username, password, userAgent, ipAddress, rememberMe });
     ...
     * 2. 委派 AuthCommandsService 執行登出業務邏輯
    ...
    public logout = async (req: Request, res: Response, next: NextFunction): Promise<void> => { // 登出控制器方法
        try {
            ...
            if (token) {
                ...
                await this.authCommandsService.logout({ token });
