 ...
 * 
 * 測試 UserCommandsService 類別的所有功能，包含：
 * - 使用者創建和驗證測試
 * - 使用者更新和密碼變更測試
 * - 使用者刪除和狀態管理測試
 * - 角色分配和移除測試
...
import { UserCommandsService, CreateUserRequest } from '../../../src/services/commands/UserCommandsService.js';
import { UserQueriesService, UserDTO } from '../../../src/services/queries/UserQueriesService.js';
import { UserCommandsRepositorysitory } from '../../../src/repo/commands/rbac/UserCommandsRepository.js';
...
// Mock repositories
jest.mock('../../../src/repo/commands/rbac/UserCommandsRepository.js');
jest.mock('../../../src/models/rbac/UserModel.js');

describe('UserCommandsService', () => {
    let service: UserCommandsService;
    let mockUserQueriesService: jest.Mocked<UserQueriesService>;
    let mockUserCommandsRepositorysitory: jest.Mocked<UserCommandsRepositorysitory>;
    ...
    beforeEach(() => {
        ...
        // Mock UserQueriesService
        mockUserQueriesService = {
            getUserById: jest.fn(),
        ...
        // Mock UserCommandsRepositorysitory
        mockUserCommandsRepositorysitory = {
            create: jest.fn(),
            update: jest.fn(),
            delete: jest.fn(),
            bulkCreate: jest.fn(),
        ...
        service = new UserCommandsService(mockUserQueriesService);
        
        // Manually set private properties for testing
        (service as any).userCommandsRepositorysitory = mockUserCommandsRepositorysitory;
    ...
    describe('createUser', () => {
        it('應該成功創建使用者', async () => {
            ...
            // Mock dependencies
            mockUserQueriesService.getUserByUsername.mockResolvedValue(null);
            mockUserQueriesService.getUserByEmail.mockResolvedValue(null);
            mockBcrypt.genSalt.mockResolvedValue('salt');
            mockBcrypt.hash.mockResolvedValue(hashedPassword);
            mockUserCommandsRepositorysitory.create.mockResolvedValue(createdUser as any);
            ...
            expect(mockUserCommandsRepositorysitory.create).toHaveBeenCalledWith({
        ...
        it('應該在使用者名稱已存在時返回錯誤', async () => {
            ...
            mockUserQueriesService.getUserByUsername.mockResolvedValue(existingUser);

            const result = await service.createUser(createRequest);

            expect(result.success).toBe(false);
            expect(result.message).toContain('使用者名稱已存在');
            expect(mockUserCommandsRepositorysitory.create).not.toHaveBeenCalled();
        });
        ...
        it('應該在電子郵件已存在時返回錯誤', async () => {
            ...
            mockUserQueriesService.getUserByUsername.mockResolvedValue(null);
            mockUserQueriesService.getUserByEmail.mockResolvedValue(existingUser);
    ...
    describe('updateUser', () => {
        it('應該成功更新使用者', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserQueriesService.getUserByEmail.mockResolvedValue(null); // email 不衝突
            mockUserCommandsRepositorysitory.update.mockResolvedValue(updatedUser as any);
            ...
            expect(result.success).toBe(true);
            expect(result.user).toBeDefined();
            expect(mockUserCommandsRepositorysitory.update).toHaveBeenCalledWith(userId, updateData);
        ...
        it('應該在使用者不存在時返回錯誤', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(null);

            const result = await service.updateUser(userId, updateData);

            expect(result.success).toBe(false);
            expect(result.message).toContain('使用者不存在');
            expect(mockUserCommandsRepositorysitory.update).not.toHaveBeenCalled();
        });
        ...
        it('應該在新電子郵件已被使用時返回錯誤', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserQueriesService.getUserByEmail.mockResolvedValue(conflictUser);
    ...
    describe('deleteUser', () => {
        it('應該成功刪除使用者', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserCommandsRepositorysitory.delete.mockResolvedValue(true);
            mockRedisClient.del.mockResolvedValue(1);
            ...
            expect(mockUserCommandsRepositorysitory.delete).toHaveBeenCalledWith(userId);
            expect(mockRedisClient.del).toHaveBeenCalledWith(`user:${userId}`);
        });
        ...
        it('應該在使用者不存在時返回錯誤', async () => {
            ...
            expect(result.success).toBe(false);
            expect(result.message).toContain('使用者不存在');
            expect(mockUserCommandsRepositorysitory.delete).not.toHaveBeenCalled();
        });
    });
    ...
    describe('changePassword', () => {
        it('應該成功更改密碼', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserCommandsRepositorysitory.findById.mockResolvedValue(userWithPassword as any);
            mockBcrypt.compare.mockResolvedValue(true);
            mockBcrypt.genSalt.mockResolvedValue('salt');
            mockBcrypt.hash.mockResolvedValue(newHashedPassword);
            mockUserCommandsRepositorysitory.update.mockResolvedValue(userWithPassword as any);
            mockRedisClient.del.mockResolvedValue(1);
            ...
            expect(mockUserCommandsRepositorysitory.update).toHaveBeenCalledWith(userId, {
    ...
    describe('activateUser', () => {
        it('應該成功啟用使用者', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserCommandsRepositorysitory.update.mockResolvedValue(activatedUser as any);
            mockRedisClient.del.mockResolvedValue(1);
    ...
    describe('deactivateUser', () => {
        it('應該成功停用使用者', async () => {
            ...
            mockUserQueriesService.getUserById.mockResolvedValue(existingUser);
            mockUserCommandsRepositorysitory.update.mockResolvedValue(deactivatedUser as any);
            mockRedisClient.del.mockResolvedValue(1);
