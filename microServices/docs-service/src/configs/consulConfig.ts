...
export class ConsulConfig {
    ...
    constructor() {
        ...
        this.serviceConfig = {
            id: 'docs-service',
            name: 'docs-service',
            address: process.env.SERVICE_HOST || 'aiot-docs-service',
            port: parseInt(process.env.HTTP_PORT || '3054'),
            tags: ['docs,documentation,api', 'microservice', 'nodejs', 'express'],
            check: {
                http: `http://${process.env.SERVICE_HOST || 'aiot-docs-service'}:${process.env.HTTP_PORT || '3054'}/health`,
                interval: '10s',
                timeout: '3s',
