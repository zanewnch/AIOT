/**
 * @fileoverview FeSetting gRPC 服務器實作
 * 
 * 此文件實作 FeSetting 服務的 gRPC 服務器，提供：
 * - 使用者偏好設定管理 gRPC 端點
 * - 系統設定管理 gRPC 端點
 * - 主題設定管理 gRPC 端點
 * - 語言設定管理 gRPC 端點
 * 
 * @module gRPC/FeSettingGrpcServer
 * @version 1.0.0
 * @author AIOT Team
 */
...
/**
 * FeSetting gRPC 服務器類別
 */
export class FeSettingGrpcServer {
  ...
  private loadProtoAndAddService(): void {
    ...
    this.server.addService(feSettingProto.fesetting.FeSettingService.service, {
      // 使用者偏好設定方法
  ...
  public start(port: number = 50053): void {
    this.server.bindAsync(
      ...
      (error, boundPort) => {
        ...
        console.log(`FeSetting gRPC server running on port ${boundPort}`);
        this.server.start();
      }
    );
  }
  ...
  public stop(): void {
    this.server.tryShutdown(() => {
      console.log('FeSetting gRPC server stopped');
    });
  }
}