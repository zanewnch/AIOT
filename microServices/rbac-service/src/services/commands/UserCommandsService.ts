...
export class UserCommandsService {
    ...
    private getUserCacheKey = (userId: number): string => {
        return `${UserCommandsService.USER_CACHE_PREFIX}${userId}`;
    }
    ...
    private hashPassword = async (password: string): Promise<string> => {
        return bcrypt.hash(password, UserCommandsService.BCRYPT_SALT_ROUNDS);
    }
    ...
    private clearUserManagementCache = async (userId?: number): Promise<void> => {
        ...
        // 清除所有使用者列表快取
        await this.safeRedisOperation(
            async (redis: RedisClientType) => await redis.del(UserCommandsService.ALL_USERS_KEY),
            'clearAllUsersCache',
            0
        );
    ...
    private cacheUser = async (user: UserDTO): Promise<void> => {
        ...
        await this.safeRedisWrite(
            async (redis: RedisClientType) => {
                await redis.setEx(key, UserCommandsService.DEFAULT_CACHE_TTL, JSON.stringify(user));
            },
            `cacheUser(${user.id})`
        );
    }
    ...
    public updateUser = async (userId: number, updateData: UpdateUserRequest): Promise<UserDTO | null> => {
        try {
            ...
            // 檢查使用者是否存在
            const existingUser = await this.userQueriesService.getUserById(userId);
            if (!existingUser) {
            ...
            if (updateData.username !== undefined) {
                ...
                // 檢查新使用者名稱是否已被其他使用者使用
                if (trimmedUsername) {
                    const usernameExists = await this.userQueriesService.usernameExists(trimmedUsername, userId);
                    if (usernameExists) {
                        throw new Error(`User with username '${trimmedUsername}' already exists`);
                    }
                }
            ...
            if (updateData.email !== undefined) {
                ...
                // 檢查新電子郵件是否已被其他使用者使用
                if (trimmedEmail) {
                    const emailExists = await this.userQueriesService.emailExists(trimmedEmail, userId);
                    if (emailExists) {
                        throw new Error(`User with email '${trimmedEmail}' already exists`);
                    }
                }
    ...
    public validateUserLogin = async (username: string, password: string): Promise<UserDTO | null> => {
        try {
            ...
            // 查找使用者（包含密碼雜湊）
            const user = await this.userQueriesService.getUserWithPasswordByUsername(username.trim());
            if (!user) {
            ...
            // 驗證密碼
            const isValidPassword = await this.userQueriesService.verifyPassword(password, user.passwordHash);
            if (!isValidPassword) {
    ...
    public verifyPassword = async (password: string, hash: string): Promise<boolean> => {
        return this.userQueriesService.verifyPassword(password, hash);
    }
}