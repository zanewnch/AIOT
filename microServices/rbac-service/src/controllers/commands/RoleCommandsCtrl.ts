...
import type { IRoleCommandsService, CreateRoleRequest, UpdateRoleRequest } from '../../services/commands/RoleCommandsSvc';
...
export class RoleCommands {
    ...
    public createRole = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (!name || typeof name !== 'string' || name.trim().length === 0) {
                const result = ControllerResult.badRequest('角色名稱不能為空');
            ...
            const result = ControllerResult.created('角色創建成功', newRole);
            res.status(result.status).json(result);
            ...
            if (error instanceof Error && error.message.includes('已存在')) {
                ...
                const result = ControllerResult.internalError('創建角色失敗');
                res.status(result.status).json(result);
            }
        }
    }
    ...
    public updateRole = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (Object.keys(updateData).length === 0) {
                const result = ControllerResult.badRequest('沒有提供更新資料');
                res.status(result.status).json(result);
                return;
            }
            ...
            const result = ControllerResult.success('角色更新成功', updatedRole);
            res.status(result.status).json(result);
            ...
            if (error instanceof Error && error.message.includes('已存在')) {
                ...
                const result = ControllerResult.internalError('更新角色失敗');
                res.status(result.status).json(result);
            }
        }
    }
    ...
    public deleteRole = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const result = ControllerResult.success('角色刪除成功');
            res.status(result.status).json(result);
            ...
            if (error instanceof Error && error.message.includes('無法刪除')) {
                ...
                const result = ControllerResult.internalError('刪除角色失敗');
                res.status(result.status).json(result);
            }
        }
    }
