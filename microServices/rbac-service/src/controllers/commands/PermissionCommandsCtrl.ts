...
import type { IPermissionCommandsService } from '../../types/services/IPermissionCommandsService';
...
export class PermissionCommands {
    ...
    public createPermission = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const result = ControllerResult.created('權限創建成功', newPermission);
            res.status(result.status).json(result);
            ...
            if (error instanceof Error && error.message.includes('已存在')) {
                ...
                const result = ControllerResult.internalError('創建權限失敗');
                res.status(result.status).json(result);
            }
        }
    }
    ...
    public updatePermission = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (error instanceof Error && error.message.includes('已存在')) {
                ...
                const result = ControllerResult.internalError('更新權限失敗');
                res.status(result.status).json(result);
            }
        }
    }
    ...
    public deletePermission = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            const result = ControllerResult.success('權限刪除成功');
            res.status(result.status).json(result);
            ...
            if (error instanceof Error && error.message.includes('無法刪除')) {
                ...
                const result = ControllerResult.internalError('刪除權限失敗');
                res.status(result.status).json(result);
            }
        }
    }
