...
export class ConsulConfig {
    ...
    constructor() {
        ...
        this.serviceConfig = {
            id: 'rbac-service',
            name: 'rbac-service',
            address: process.env.SERVICE_HOST || 'aiot-rbac-service',
            port: parseInt(process.env.HTTP_PORT || '3051'),
            tags: ['rbac', 'authorization', 'roles', 'permissions', 'microservice', 'nodejs', 'express'],
            check: {
                http: `http://${process.env.SERVICE_HOST || 'aiot-rbac-service'}:${process.env.HTTP_PORT || '3051'}/health`,
                interval: '10s',
                timeout: '3s',
