...
本模組實現 LLM Service (Django) 的 Consul 服務註冊功能，
支援服務健康檢查、自動註冊和註銷機制。
...
class ConsulConfig:
    ...
    負責管理 LLM Service (Django) 在 Consul 中的註冊、註銷和健康檢查。
    """
    ...
    def __init__(self) -> None:
        ...
        service_port = int(os.getenv('PORT', '8022'))
        ...
        self.service_config = ServiceConfig(
            ...
            port=service_port,
            tags=[
                'llm', 'django', 'rest-api', 'transformers',
                'microservice', 'python', 'docs-service'
            ],
            check={
                'http': f"http://{service_host}:{service_port}/api/transformers/health/",
                'interval': '10s',
                'timeout': '5s',
    ...
    def register_service(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(
                    f"✅ LLM Service registered to Consul: "
            ...
            logger.error(f"❌ Failed to register LLM Service to Consul: {e}")
    ...
    def deregister_service(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"✅ LLM Service deregistered from Consul: {self.service_config.id}")
            else:
                logger.error(f"❌ Failed to deregister from Consul: HTTP {response.status_code}")
                
        except Exception as e:
            logger.error(f"❌ Failed to deregister LLM Service from Consul: {e}")
