...
const getServiceType = (): ServiceType => {
  const serviceName = process.env.SERVICE_NAME || 'drone-websocket-service';
...
export const getDatabaseConfig = (): DatabaseConfig => ({
  ...
  // 從環境變數獲取資料庫名稱，預設為 drone_db（Drone WebSocket Service 專用）
  database: process.env.DB_NAME || 'drone_db',
  // 從環境變數獲取資料庫使用者名稱，預設為 admin
  username: process.env.DB_USER || 'admin',
  // 從環境變數獲取資料庫密碼，預設為 admin
  password: process.env.DB_PASSWORD || 'admin',
  // 從環境變數獲取資料庫埠號並轉換為整數，預設為 5434（Drone WebSocket Service 專用）
...
export const createSequelizeInstance = (): Sequelize => {
  ...
  const sequelize = new Sequelize({
    // 展開資料庫配置物件的所有屬性
    ...config,
    // 註冊 Drone WebSocket Service 相關模型到 Sequelize 實例中
    models: [
      DronePositionModel, 
      DroneStatusModel, 
      DroneRealTimeStatusModel, 
      DroneCommandModel, 
      DroneCommandQueueModel, 
      DroneCommandsArchiveModel, 
      DronePositionsArchiveModel, 
      DroneStatusArchiveModel, 
      ArchiveTaskModel
    ],
