/**
 * @fileoverview Logger Decorator Pattern 實現 - Drone WebSocket Service
 * 
 ...
 * - 支援錯誤記錄和執行時間統計
 * - 可組合和可擴展
 * - 適配 Drone WebSocket Service 的 logger 系統
 * 
 * @author AIOT Team
 * @version 1.0.0
 * @since 2025-08-13
...
export class LoggerDecorator implements Component {
    ...
    private isControllerMethod = (methodName: string): boolean => {
        const controllerMethods = ['create', 'update', 'delete', 'get', 'list', 'find', 'health', 'status'];
...
/**
 * 工廠方法：創建帶有日誌功能的 WebSocket 服務
 */
export const createLoggedWebSocketService = <T extends Component>(
    service: T, 
    className: string, 
    options?: LoggerOptions
): T => {
