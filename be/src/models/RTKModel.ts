 ...
 * 此介面確保型別安全和資料完整性。
 * 
 * @interface RTKAttributes
 * @since 1.0.0
 */
export type RTKAttributes = {
    /** 
     * 主鍵識別碼
 ...
 * 繼承自 RTKAttributes 並將 id 設為可選屬性。
 * 
 * @interface RTKCreationAttributes
 * @extends {Optional<RTKAttributes, 'id'>}
 * @since 1.0.0
 */
export type RTKCreationAttributes = Optional<RTKAttributes, 'id' | 'createdAt' | 'updatedAt'>;
 ...
 * 
 * @class RTKModel
 * @extends {Model<RTKAttributes, RTKCreationAttributes>}
 * @implements {RTKAttributes}
 * @since 1.0.0
 * 
 * @example
 * ```typescript
 * // 建立新的 RTK 資料記錄
 * const rtkData = await RTKModel.create({
 *   latitude: 25.033964,  // 台北市政府緯度
 *   longitude: 121.564468 // 台北市政府經度
 * });
 * 
 * // 查詢最新的定位資料
 * const latestData = await RTKModel.findOne({
 *   order: [['createdAt', 'DESC']]
 * });
 * ```
 */
@Table({ tableName: 'rtk_data', timestamps: true }) // 設定資料表名稱為 'rtk_data'，啟用時間戳記
export class RTKModel extends Model<RTKAttributes, RTKCreationAttributes> implements RTKAttributes {
    /**
     * 主鍵識別碼
     * 
