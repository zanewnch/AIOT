...
// 匯入身份驗證服務介面和相關類型
import { IAuthService, LoginResult } from '../types/services/IAuthService.js';
...
export class AuthService implements IAuthService {
     ...
     */
    constructor() { // 建構函式，接受可選的使用者資料存取層實例
        this.userRepository = new UserRepository(); // 設定使用者資料存取層，用於查詢使用者資料
    }
    ...
    async login(username: string, password: string, userAgent?: string, ipAddress?: string): Promise<LoginResult> { // 異步方法：執行完整的使用者登入驗證流程
        try { // 嘗試執行登入驗證流程
            ...
            if (!user) { // 如果找不到使用者
                ...
                return { success: false, message: 'Invalid credentials' }; // 回傳登入失敗結果，避免洩露使用者是否存在的資訊
            }
            ...
            if (!match) { // 如果密碼不匹配
                ...
                return { success: false, message: 'Invalid credentials' }; // 回傳與使用者不存在相同的錯誤訊息，避免資訊洩露
            }
            ...
            return { success: true, message: 'Login successful', token, user }; // 回傳登入成功結果
        } catch (error) { // 捕獲整個登入流程中的任何未預期錯誤
            logger.error('Login error occurred:', error); // 記錄錯誤日誌
            return { success: false, message: 'Login failed' }; // 回傳通用的登入失敗結果
        }
    }
