...
export class UserToRoleService { // 使用者角色關聯服務類別，提供完整的使用者角色管理功能
     ...
     */
    public async getAllUserRoles(): Promise<Array<{ id: string, userId: number, roleId: number, assignedAt: string }>> { // 公開異步方法：取得所有使用者角色關聯數據
        try { // 嘗試執行取得所有使用者角色關聯操作
            logger.info('Getting all user-role associations'); // 記錄開始取得所有使用者角色關聯的資訊日誌

            // 從資料庫取得所有使用者角色關聯，不包含關聯的使用者和角色資訊以避免關聯錯誤
            const userRoles = await this.userRoleRepository.findAll(false); // 調用使用者角色資料存取層取得基本關聯資料

            // 轉換為簡化的 DTO 格式
            const userRolesDTO = userRoles.map((ur: any) => ({ // 將使用者角色關聯轉換為簡化的 DTO 格式
                id: `${ur.userId}-${ur.roleId}`, // 組合 ID，避免使用可能不存在的資料庫 ID
                userId: ur.userId, // 使用者 ID
                roleId: ur.roleId, // 角色 ID
                assignedAt: (ur.createdAt || new Date()).toISOString() // 分配時間轉為 ISO 字串格式
            }));

            logger.info(`Retrieved ${userRolesDTO.length} user-role associations`); // 記錄取得使用者角色關聯數量的資訊日誌
            return userRolesDTO; // 回傳簡化的使用者角色關聯 DTO 列表
        } catch (error) { // 捕獲過程中的任何錯誤
            logger.error('Error getting all user roles:', error); // 記錄取得所有使用者角色關聯失敗的錯誤日誌
            if (error instanceof Error) { // 如果是 Error 類型的錯誤
                throw error; // 直接重新拋出，保持原始錯誤訊息
            }
            throw new Error('Failed to get all user roles'); // 拋出通用錯誤訊息
        }
    }
}