 ...
 * API 函數：獲取角色權限關聯
 */
export const getRoleToPermissionAPI = async (roleId: number): Promise<Permission[]> => {
  try {
 ...
 * API 函數：更新角色資料
 */
export const updateRoleAPI = async (id: number, data: RoleUpdateRequest): Promise<UpdateResponse> => {
  try {
...
export const useRolePermissions = (roleId: number, enabled: boolean = true) => {
  return useQuery({
    ...
    queryFn: () => getRoleToPermissionAPI(roleId),
