...
export const useUserPreferences = () => {
  return useQuery({
    queryKey: USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES,
...
export const useCreateUserPreferences = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      // 更新查詢緩存
      queryClient.setQueryData(USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES, data);
    },
    onError: (error) => {
...
export const useUpdateUserPreferences = () => {
  ...
  return useMutation({
    ...
    onMutate: async (newPreferences) => {
      // 取消任何正在進行的查詢
      await queryClient.cancelQueries({ queryKey: USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES });

      // 保存當前資料作為回滾使用
      const previousPreferences = queryClient.getQueryData(USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES);

      // 樂觀更新
      queryClient.setQueryData(
        USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES,
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: USER_PREFERENCE_QUERY_KEYS.USER_PREFERENCES });
    },
    retry: 2,
  });
};
...
export const usePreferencesManager = () => {
  ...
  const updateMutation = useUpdateUserPreferences();
  const quickUpdate = useUpdatePreference();
