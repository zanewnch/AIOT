...
export const logRequest = (
  ...
  if (level === 'silent') return; // Skip silent level
  const logMethod = logger[level as keyof ExtendedLogger] as (message: string, ...args: any[]) => void;
  logMethod(message, { url, method, timestamp: formatTimestamp() });
};
...
export const logError = (
  ...
  logger.error(`Error in ${context || 'unknown context'}: ${errorMessage}`, {
    error: errorMessage,
    stack: errorStack,
    context,
    additionalData,
